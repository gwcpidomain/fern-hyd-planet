...
export function ForecastTile({ forecast, isLoading }: ForecastTileProps) {
  ...
  return (
    <Card className="dash-tile relative h-full min-h-0 gap-0 overflow-hidden rounded-2xl border-white/[0.11] bg-[rgba(8,15,38,0.45)] p-0 text-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.22)]">
        ...
        {isEmpty ? (
          ...
          days.map((day, index) => {
            ...
            return (
              <div
                key={day.date}
                className="flex w-full min-w-0 items-center gap-2 rounded-2xl border border-white/[0.06] bg-white/[0.04] px-2.5 py-2.5 backdrop-blur-sm transition-colors hover:bg-white/[0.06] sm:gap-3"
              >
